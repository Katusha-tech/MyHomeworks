...
list_user_input: List = list(map(lambda i: (int(i) if i.isdigit() else None), user_input)) #Создали список, где заменили число -числом, а не число на None 
...
filtered_movies: Dict[Any, Dict[str, Any]]= filter(lambda item: item[1]["title"] and item[1]["title"].startswith("Ч"), full_dict.items())
