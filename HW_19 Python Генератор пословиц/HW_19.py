...
number = int(input('Сколько пословиц нужно?'))
proverbs_new = set(proverbs)
variants_new = set(variants)
results = set()

while (proverbs_new and variants_new) and len(results) < number:
    random_variant = variants_new.pop()
    random_proverb = proverbs_new.pop()

    proverb_var = random_proverb.lower().replace('ум', random_variant)
    results.add(proverb_var)
